template<typename T>
VPPointerTree<T>::VPPointerTree(const MetricFunctor<T> & metric, typename std::vector<T> & items):
    Spatial<T>(metric),
    points_(&items),
    root_(nullptr)
{
    this->root_ = this->build(this->points_->begin(), this->points_->end());
    this->size_ = items.size();
}

template<typename T>
VPPointerTree<T>::~VPPointerTree(void) {
    delete this->root_;
}

template<typename T>
VPNode<T*> * VPPointerTree<T>::build(typename std::vector<T>::iterator begin, typename std::vector<T>::iterator end) {
    if (begin >= end) {
...
template<typename T>
QueueItem<T*> VPPointerTree<T>::find_nearest(const T & point) const {
...
template<typename T>
std::vector<QueueItem<T*>> VPPointerTree<T>::find_nearest(const T & point, unsigned int count) const {
...
template<typename T>
std::vector<QueueItem<T*>> VPPointerTree<T>::find_within(const T & point, real radius) const {
    std::vector<QueueItem<T*>> result;
 ...
 * **/
template<typename T>
void VPPointerTree<T>::search_one(VPNode<T*> * node, const T & target, QueueItem<T*> & candidate) const {
    if (node != nullptr) {
...
template<typename T>
void VPPointerTree<T>::search_queued(VPNode<T*> * node, const T & target, unsigned int count, std::priority_queue<QueueItem<T*>> & pq) const {
    if (node != nullptr) {
        real distance = this->metric_(*(node->point), target);
...
template<typename T>
void VPPointerTree<T>::search_radius(VPNode<T*> * node, const T & target, real radius, std::vector<QueueItem<T*>> & result) const {
    if (node != nullptr) {
        real distance = this->metric_(*(node->point), target);

        if (distance < radius) {
            result.push_back(QueueItem<T*>(node->point, distance));
        }
...
template<typename T>
void VPPointerTree<T>::print(void) const {
