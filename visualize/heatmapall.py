...
class HeatmapAll(heatmap.Heatmap):
    def __init__(self):
        super().__init__()
...
#            ax.axis('off')
    ...
    def time_frames(self):
        ...
        for frame in range(0, self.args.lsun):
            ...
            ax.set_title(f'The global meteor PDF (at λ = 261.5°)')
            ...
            fig.colorbar(im, ax=ax, shrink=0.9, aspect=30*0.9, label='ln(1 + Â)', pad=0.01)

            if self.args.points:
                ax.scatter(self.points['ra'], self.points['dec'], s=0.02, marker='o', c=self.points['sunlon'] / 360, cmap='hsv')

            fig.patch.set_facecolor('white')
            plt.tight_layout(rect=(0, 0, 1.08, 1))
            fig.savefig(self.outdir / f'{frame:03d}.png', dpi=300, format='png')
            print(f"[{frame:03d}]", end='\n' if frame + 1 == self.args.lsun else ' ', flush=True)
            plt.close('all')
